...
var d3 = Object.assign({}, require('d3-color'), require('d3-hierarchy'), require('d3-selection'), require('d3-shape'));

var d3SvgSingleArc = require('../svg-single-arc');
...
function renderChart(widget) {
    ...
    function twoArcs(g) {
      g.each(function (d) {
        ...
        if (!d.children) {
          ...
          var _bar = d3.arc().startAngle(d.x0).endAngle(d.x1).innerRadius(inner_r).outerRadius(out_r); // Draw leaf arcs

          widget._childElements.parts[widget.getUniqueId(d, 'bar')].attr('d', _bar);
          ...
          widget._childElements.parts[widget.getUniqueId(d, 'sector_leaf_text')].attr('dy', dy_init).attr('text-anchor', lbl_anchor).attr('transform', 'translate(' + (out_r + 5) * Math.sin(d.x0) + ',' + -(out_r + 5) * Math.cos(d.x0) + ') ' + 'rotate(' + rotate + ')');
        } else {
          var arc1_extend = Math.max(Math.abs(score) * (d.x1 - d.x0) - right_pad, 0);
          //Regular renders
          var arc1 = d3.arc().startAngle(function (d) {
            return d.x0;
          }) //x:startangle,
          .endAngle(function (d) {
          ...
          widget._childElements.parts[widget.getUniqueId(d, 'arc1')].attr('d', arc1);

          var arc2 = d3.arc().startAngle(function (d) {
            return d.x0 + arc1_extend;
          }) //x:startangle,
          .endAngle(function (d) {
          ...
          widget._childElements.parts[widget.getUniqueId(d, 'arc2')].attr('d', arc2);
          ...
          if (bottom) {
            ...
            arc_for_label = d3SvgSingleArc().startAngle(function (d) {
              return d.x0;
            }).endAngle(function (d) {
            ...
            arc_for_label_number = d3SvgSingleArc().startAngle(function (d) {
              return d.x0;
            }).endAngle(function (d) {
              return d.x1 - right_pad;
            }).radius(function (d) {
          ...
          widget._childElements.parts[widget.getUniqueId(d, 'arc_for_label')].attr('d', arc_for_label);
          if (d.depth > 1) {
            widget._childElements.parts[widget.getUniqueId(d, 'arc_for_label_number')].attr('d', arc_for_label_number);
          }
        }
      });
    }
  ...
  widget.vis = widget.d3vis.append('g').attr('transform', 'translate(' + widget.dimW / 2 + ',' + widget.dimH / 2 + ')').append('g');
  ...
  var root = d3.hierarchy(profile).sum(function (d) {
  ...
  var g = widget.vis.selectAll('g').data(partition(root).descendants()).enter().append('g').attr('class', 'sector').attr('visibility', function (d) {
    return d.depth === 2 || d.forceVisible ? 'visible' : 'hidden';
  }) // hide non-first level rings
  .call(sector).each(function (d) {
    d.expand = 1;
  }).on('click', function (d) {
