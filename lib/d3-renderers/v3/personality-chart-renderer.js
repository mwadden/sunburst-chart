...
var d3 = require('d3');
Object.assign(d3, require('d3-color'));
...
function renderChart(widget) {
    ...
    function twoArcs(g) {
      g.each(function (d) {
        ...
        if (!d.children) {
          ...
          widget._childElements.parts[widget.getUniqueId(d, 'sector_leaf_text')].attr('dy', dy_init).attr('text-anchor', lbl_anchor).attr('transform', 'translate(' + (out_r + 5) * Math.sin(d.x) + ',' + -(out_r + 5) * Math.cos(d.x) + ') ' + 'rotate(' + rotate + ')');
        } else {
  ...
  var g = widget.vis.data([profile]).selectAll('g').data(partition.nodes).enter().append('g').attr('class', 'sector').attr('visibility', function (d) {
