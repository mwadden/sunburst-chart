...
var utils = require('./d3-renderers/utils');

var SunburstWidget = function () {
  function SunburstWidget(options, renderer) {
    _classCallCheck(this, SunburstWidget);
  ...
  _createClass(SunburstWidget, [{
    ...
    value: function init() {
      ...
      this.d3vis.attr('width', this.visualizationWidth).attr('height', this.visualizationHeight).attr('viewBox', '0 -25 ' + this.dimW + ' ' + (this.dimH + 25));
    }
  }, {
    key: 'clear',
    ...
    key: 'updateLabelLayout',
    value: function updateLabelLayout() {
      this.updateLabelLayoutWithClass('.sector_label_path');
      this.updateLabelLayoutWithClass('.sector_label_number_path');
    }
  }, {
    key: 'updateLabelLayoutWithClass',
    value: function updateLabelLayoutWithClass(_class) {
      if (!this.d3vis) {
        return;
      }
      ...
      this.d3vis.selectAll(_class).each(function (d) {
        ...
        if (text && text.length > 0) {
          ...
          var sector_length = self._d3version === 'v3' ? (d.y + d.dy * frac) * d.dx : (d.y0 + (d.y1 - d.y0) * frac) * (d.x1 - d.x0);
    ...
    key: 'updatePartColor',
    value: function updatePartColor(d) {
      ...
      if (!d.children) {
        this._childElements.parts[this.getUniqueId(d, 'bar')].style('stroke', '#EEE').style('fill', this.d3.color(colors.outerRingColor));
      } else {
        this._childElements.parts[this.getUniqueId(d, 'arc1')].style('stroke', strokecolor).style('fill', arc1color);

        this._childElements.parts[this.getUniqueId(d, 'arc2')].style('stroke', strokecolor).style('fill', arc1color).style('fill-opacity', 0.15);
      }
    }
  }, {
    key: 'addImage',
    value: function addImage(url) {
      ...
      if (!this.hasImage()) {
        ...
        var pattern = this._childElements.icondefs.append('pattern');
        pattern.attr('id', id).attr('height', 1).attr('width', 1).attr('patternUnits', 'objectBoundingBox');

        this._childElements.image = pattern.append('image');

        this._childElements.image.attr('width', scaled_w).attr('height', scaled_w).attr('x', 0).attr('y', 0).attr('opacity', 1.0).on('dblclick.zoom', null);

        this._childElements.circle = this.vis.append('circle');

        this._childElements.circle.attr('r', radius).attr('stroke-width', 0).attr('fill', 'url(#' + id + ')');
      }
    ...
    key: 'getScoreLabelText',
    value: function getScoreLabelText(d) {
      var score = utils.getValue(d, 'score');
      return score === null || isNaN(score) ? '' : ' (' + (this.getScore(d) * 100).toFixed(0) + '%)';
    }
  }, {
    key: 'getNameLabelText',
    value: function getNameLabelText(d) {
      var name = utils.getValue(d, 'name');
      if (!name) {
        return '';
      }
    ...
    key: 'getUniqueId',
    value: function getUniqueId(d, _class) {
    ...
    key: 'createParts',
    value: function createParts(g, d) {
      var self = this;
      var uid;
      ...
      if (!d.children) {
        ...
        if (!this._childElements.parts[uid]) {
          this._childElements.parts[uid] = g.append('path').attr('class', '_bar');
        }

        uid = this.getUniqueId(d, 'sector_leaf_text');
        if (!this._childElements.parts[uid]) {
          this._childElements.parts[uid] = g.append('text').attr('class', 'sector_leaf_text');
        }
      } else {
        uid = this.getUniqueId(d, 'arc1');
        if (!this._childElements.parts[uid]) {
          this._childElements.parts[uid] = g.append('path').attr('class', 'arc1');
        }

        uid = this.getUniqueId(d, 'arc2');
        if (!this._childElements.parts[uid]) {
          this._childElements.parts[uid] = g.append('path').attr('class', 'arc2');
        }

        uid = this.getUniqueId(d, 'arc_for_label');
        if (!this._childElements.parts[uid]) {
          this._childElements.parts[uid] = g.append('path').attr('class', 'arc_for_label').attr('id', function (d) {
        ...
        if (!this._childElements.parts[uid]) {
          this._childElements.parts[uid] = g.append('text').attr('class', 'sector_label').attr('visibility', function (d) {
        ...
        if (d.depth > 1) {
          ...
          if (!this._childElements.parts[uid]) {
            this._childElements.parts[uid] = g.append('path').attr('class', 'arc_for_label_number').attr('id', function (d) {
          ...
          if (!this._childElements.parts[uid]) {
            this._childElements.parts[uid] = g.append('text').attr('class', 'sector_label_number ').attr('visibility', function (d) {
              return d.depth === 1 ? 'visible' : null;
            }).attr('class', 'sector_nonleaf_text').append('textPath').attr('class', 'sector_label_number_path').attr('position-in-sector', bottom ? 'outer' : 'inner') // Since both text lines share the same 'd', this class annotation tells where is the text, helping to determine the real arc length
